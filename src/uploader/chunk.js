...
function Chunk (uploader, file, offset) {
  ...
  this.offset = offset // 文件的第几块
  this.tested = false // 是否已经发送过测试请求
  this.retries = 0  // 重试次数
  this.pendingRetry = false // 重试中
  this.preprocessState = 0  // 上传前为0 会调用preprocess方法，执行send后会置为1
  this.readState = 0  // 上传前为0 会调用readFileFn方法， readFinished执行后会置为2
  this.loaded = 0 // 已上传大小
  this.total = 0  // 总大小
  this.chunkSize = utils.evalOpts(uploader.opts.chunkSize, file, this) // 块大小
  this.startByte = this.offset * this.chunkSize // 开始字节
  this.endByte = this.computeEndByte()  // 结束字节
  this.xhr = null
}
...
utils.extend(Chunk.prototype, {
  ...
  computeEndByte: function () {
    ...
    // 剩余上传大小 < 块大小 & 不强制要求块小于等于chunkSize时
