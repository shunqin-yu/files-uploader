...
Uploader.defaults = {
  ...
  simultaneousUploads: 3, // 并发上传数量
  singleFile: false,  // 单文件上传
  fileParameterName: 'file',  // 上传时文件的参数名
  progressCallbacksInterval: 500, // 上传进度回调间隔
  speedSmoothingFactor: 0.1,  // 用于计算平均速度，0 - 1，1等于当前上传速度，大文件建议0.2
  query: {},  // 请求时携带其他额外的参数
  headers: {},  // 请求时携带的请求头
  withCredentials: false, // 请求是否携带cookie
  preprocess: null, // 每个块上传前调用的方法，函数内需要调用preprocessFinished以继续
  method: 'multipart',  // 请求时的content-type，octet | multipart
  testMethod: 'GET',  // 测试请求时的方法
  uploadMethod: 'POST', // 上传请求时的方法
  prioritizeFirstAndLastChunk: false, // 是否优先发送第一个和最后一个块，一般用于服务端判断文件是否合法
  allowDuplicateUploads: false, // 已上传的文件是否还允许再次上传
  target: '/',  // 目标上传URL，也可以是函数
  testChunks: true, // 是否测试每个块是否在服务端已经上传了，用来实现秒传、跨浏览器上传
  generateUniqueIdentifier: null, // 可覆盖默认的生成文件唯一标识的函数
  maxChunkRetries: 0, // 最大失败重传次数
  chunkRetryInterval: null, // 失败重传间隔
  permanentErrors: [404, 415, 500, 501], // 出错的响应码
  successStatuses: [200, 201, 202], // 成功的响应码
  onDropStopPropagation: false, // 
  initFileFn: null, // Function(Uploader.File实例), 用于初始化文件对象
  readFileFn: webAPIFileRead,
  checkChunkUploadedByResponse: null, // Function(Uploader.Chunk) 用于根据XHR响应内容检测每个块是否上传成功了
  initialPaused: false, // 初始时是否暂停
